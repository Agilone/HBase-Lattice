...
public class RawScanResult implements Cloneable, Writable {
	...
	public void mergeMeasuresSubgroups(RawScanResult other, byte[] thisKey, byte[] otherKey, AggregateFunctionRegistry afr, SliceOperation so) {
		for (int i = 0; i < measures.length; i++) {
			if (other.measures[i] != null) {
				if (measures[i] == null) {
					measures[i] = other.measures[i];
		...
		if(keyOffset != null) {
			...
			System.arraycopy(thisKey, keyOffset[0][keyOffset[0].length-1], row1, 0, row1.length);
			System.arraycopy(otherKey, keyOffset[0][keyOffset[0].length-1], row2, 0, row1.length);
